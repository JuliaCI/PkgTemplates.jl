...
"""
@plugin struct DowngradeCI <: FilePlugin
...
source(p::DowngradeCI) = p.file
destination(p::DowngradeCI) = joinpath(".github", "workflows", p.destination)
tags(::DowngradeCI) = "<<", ">>"

badges(p::DowngradeCI) = p.badge ? Badge(
    "Downgrade",
    "https://github.com/{{{USER}}}/{{{PKG}}}.jl/actions/workflows/$(p.destination)/badge.svg?branch={{{BRANCH}}}",
    "https://github.com/{{{USER}}}/{{{PKG}}}.jl/actions/workflows/$(p.destination)?query=branch%3A{{{BRANCH}}}",
) : Badge[]

function view(p::DowngradeCI, t::Template, pkg::AbstractString)
    v = Dict(
        "PKG" => pkg,
        "USER" => t.user,
        "SKIP" => join(p.skip, ","),
...
needs_username(::DowngradeCI) = true
customizable(::Type{DowngradeCI}) = ()  # Override AllCI default (no extra_versions field)
...
const AllCI = Union{AppVeyor, GitHubActions, TravisCI, CirrusCI, GitLabCI, DroneCI, DowngradeCI}
